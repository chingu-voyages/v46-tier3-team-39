...
export async function generateQuestion(req: Request) {
  try {
    ...
    switch (type) {
      case "Multiple Choice":
        questionType = `A Multiple choice question, with ${numberOfOptions} unique options, but only one correct answer`;
      case "Short Answer":
        questionType = "Short Answer question";
      case "Select Multiple":
        questionType = `A Select Multiple/Select All question, with ${numberOfOptions} unique options, where each option can be a correct answer OR incorrect answer. However, there MUST be at least ONE correct answer`;
      default:
        questionType = "Short Answer question";
    }
    ...
    Then, store the new question description as a string called 'QuestionDescription', the new question title as a string 'QuestionTitle', ${determineVariableTypeTemplateStr(
      type,
      numberOfOptions
    )}
